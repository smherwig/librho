 ...
 */
const char *
rho_bearssl_error_get_name(int err, const char **comment)
{
	size_t u;
...
void
rho_bearssl_warn_last_error(const br_ssl_engine_context *engine, 
        const char *fmt, ...)
{
    int error = 0;
    ...
    fputs("\n", stderr);
    va_end(ap);
    error = br_ssl_engine_last_error(engine);
    name = rho_bearssl_error_get_name(error, &comment);

    fprintf(stderr, "BearSSL Error: %d:%s:%s\n", error, name, comment);
}
