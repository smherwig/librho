...
/*
 * rho_strlcpy and rho_strlcat are taken from OpenBSD 6.2's
 * lib/libc/string/strlcpy.c and lib/libc/string/strlcat.c, respectively.
 * The license from those sources is reproduced below:
 */
 ...
 */
size_t
rho_strlcpy(char *dst, const char *src, size_t dsize)
{
	const char *osrc = src;
	size_t nleft = dsize;
 ...
 */
size_t
rho_strlcat(char *dst, const char *src, size_t dsize)
{
	const char *odst = dst;
	const char *osrc = src;
	size_t n = dsize;
